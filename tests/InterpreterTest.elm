    ...
    describe "Interpreter suite"
            ...
            , test "summation with undefined variables" <|
                \_ ->
                    parseAndRun "\\sum_{x=1}^{7} y + (1 + 1)"
                        |> isEq (Return.Expression (TripleArity (Sum_ "x") (Number 1) (Number 7) (DoubleArity Addition (Variable (ScalarIdentifier "y")) (Number 2))))
            ]
        ...
        , describe "assignments" <|
            ...
            , test "returns unapplied expression if the variable is not defined" <|
                \_ ->
                    parseAndRun "x + 1"
                        |> isEq (Return.Expression (DoubleArity Addition (Variable (ScalarIdentifier "x")) (Number 1)))
            , test "applies the parts that can be calculated" <|
                \_ ->
                    parseAndRun "x + (1 + 1)"
                        |> isEq (Return.Expression (DoubleArity Addition (Variable (ScalarIdentifier "x")) (Number 2)))
